...
2. **面板落点 = `sidebar.footer.action`（list/root）+ 自绘对话框**。理由是两个运行时的交集只有一个 additive 的侧栏座位；面板自带内联样式与纯 React 组件（不引组件库、不新增 CSS 管线），因此不需要与宿主共享任何运行时身份。`sidebar.panellist` + `main` 的"全屏面板"升级留待只支持 0.1.5 时再做。
