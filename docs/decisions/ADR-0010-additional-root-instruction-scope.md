...
- 目录考察的代价有上界：每一步考察的目录 = 根 ∪ 已投递 nested scope 所在目录 ∪ 本次触碰路径的父目录（去重）；每 session 的投递 scope 上限 512、待用触碰上限 64，触顶即停发 nested 并只告警一次（"降级但正确"）。
