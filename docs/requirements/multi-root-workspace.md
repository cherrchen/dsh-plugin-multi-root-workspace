...
| （补充）附加根自己的指令文件 | `multi-root-instructions` 行：以每个附加根自身为 `cwd`/`projectRoot` 发现**顶层** `AGENTS.md` / `CLAUDE.md`，经 `agent/pre-step` 以 `{ kind: 'plugin', form: 'instructions' }` 的 user 消息注入，全部附加根共享 64 KiB 预算，根离场即显式撤销（[ADR-0010](../decisions/ADR-0010-additional-root-instruction-scope.md)） | H4 Phase 1（已实现）；**nested instructions 不在本期**，见 §4 第二期 |
...
- **H3 DSH 兼容性代码契约**：支持矩阵是精确版本 allowlist（当前 `0.1.5-rc.2`、`0.1.6-alpha.1`），启动时校验版本与混装，判定失败则四个安全相关行根本不启动；版本差异集中在 `src/compat/`。见 [ADR-0009](../decisions/ADR-0009-dsh-compat-contract.md)。
...
| P1 | **H4 Phase 2：附加根 nested instructions** | H4 的第二个 Phase；需先评估 0.1.6 起的 `SessionMessageProjection`，再决定如何复用 primary 的 touched-path reconcile 语义（方案草案见 [compat 计划 §5b](../plans/completed/2026-09-15-dsh-compat-contract.md)） |
...
7.8 **附加根指令注入（H4 Phase 1）**：零附加根时该行一条 message 都不注入（空根与未装插件仍逐字节一致）；每个附加根**顶层**的 `AGENTS.md` / `CLAUDE.md` 在模型第一步之前可见，且以绝对路径标注（两个根的 `AGENTS.md` 不混淆）；所有附加根共享同一份 64 KiB 预算；内容未变不重复注入，变化后只重发变化的那一个根；根被移除、消失或被替换（`redirected`）时注入**显式撤销**文案；`$DSH_HOME`、主根自身与任何祖先目录的指令文件不被重复注入。见 [ADR-0010](../decisions/ADR-0010-additional-root-instruction-scope.md)。
...
- 附加根只注入**顶层**指令文件：子目录里的 nested `AGENTS.md` / `CLAUDE.md` 不会进入模型上下文（H4 Phase 2，见 §4 第二期）。
...
- 不重新实现上游 `agent-instructions` 的 session projection / 增量 reconcile 机制：附加根的 nested instructions（H4 Phase 2）在完成 `SessionMessageProjection` 评估之前不做，也不接受"直接复用 primary 语义"的临时实现。
...
- ~~附加根的 `AGENTS.md` 对模型不可见~~ → 顶层注入 + 显式撤销，见 [ADR-0010](../decisions/ADR-0010-additional-root-instruction-scope.md)（§5 验收 7.8）；nested instructions 仍开放，见 §4 第二期。

仍然开放：

- 命令输出文案的 host 侧本地化（需要 host 侧的语言来源）。
- 第二期的全部范围（LSP 路由、Windows 内核级多根、`workspace-files` 多根、nested instructions 等）见 §4 的优先级表与[路线图 §风险](../plans/active/2026-09-12-multi-root-workspace.md#风险与开放问题)。
